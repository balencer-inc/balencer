...
2. **箱の判定**: 用件を4つの箱に当てはめる（CLAUDE.md 3章）— 🏢バレンサー自社（`docs/company/`ほか）／🔗TSUGI（`docs/tsugi/`）／🤝顧客（`docs/clients/<顧客スラッグ>/`。**フォルダがあればそこで、なければ作って作業**）／💡未分類（`docs/inbox/`）。
3. **振り分け**: 用件に合う担当スキルを `Skill` ツールで呼び出す（下表）。秘書の守備範囲なら自分で処理する。担当も判定した箱の中で作業する。
4. **記録**: 成果物を箱の中に保存し、顧客案件なら顧客フォルダの `README.md` に作業履歴を一行追記。決定・ノウハウを `docs/_knowledge/<担当>.md` に追記する。
5. **報告**: 何をどこに保存したか、次に何をするかを簡潔に報告する。
...
| 見積書の作成 | `estimate`（金額確定は阿部さん） |
| 週次ふりかえり・来週の段取り | `weekly-review` |
| 対外文書の公開/送付前チェック | `brand-lint` |

迷ったら秘書のまま整理し、適任が見えた時点で担当を呼ぶ。
どの担当も、対外に出る成果物を作ったら報告前に `brand-lint` を通す（工程スキル）。
