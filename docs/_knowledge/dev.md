...
## 2026-09-03 追記2：NotebookLM の裏取りで結論が1つ変わった（HASSIN制作システム）

Web検索だけで出した結論を NotebookLM の deep research（77ソース）で裏取りしたら、**画像基盤の答えが変わった**。「調べ直す価値がある/ない」の判断材料として残す。

### 変わった点は「選択肢の存在を知らなかった」こと
Web検索では Cloudinary vs R2 vs Vercel Blob vs Drive の比較しか出てこなかったが、**Cloudflare Images の Remote Images / Transformations** という使い方が出てきた。
- Images に画像を**保存**すると 保存$5/10万枚＋配信$1/10万枚＋変換 がかかる
- **外部（R2）をオリジンにして変換だけ使うと、変換数のみの課金。月5,000ユニーク変換まで無料**
- 同一画像に同じパラメータのリクエストはキャッシュ配信で、2回目以降はカウントされない

→ **Cloudinary の役割（リサイズ・WebP/AVIF・CDN配信）をそのまま置き換えられて、実質無料。** 単価表の比較だけでは辿り着かなかった。
教訓: **「AとBどちらが安いか」で調べると、AでもBでもない組み合わせを見落とす。** 単価比較の前に「この機能を満たす構成は何通りあるか」を調べる。

### 数字が微妙にズレていた（方向は変わらないが提示前に効く）
- Cloudinary の実効クレジット単価: Web検索の $0.38 → 実際は **$0.415〜0.44**（Plus $99/225・Advanced $249/600）。超過は $0.40〜0.45
- Drive の `thumbnailLink`: 「約1日」→ 実際は **数時間**
- Drive API のクォータ: 「60秒12,000クエリ」→ **2026-05-01 から quota units モデル**（プロジェクト1分100万・ユーザー1分32.5万・1日1TBエグレス上限。リスト100・ダウンロード200ユニット）。旧クエリ数は旧プロジェクト向け

### 制約を1つ見落としていた（これは結論に効いた）
Drive を画像配信に使えない理由として `thumbnailLink` の失効とクォータを挙げていたが、**CORS制限がかかるよう設計されている（公式にプロキシ推奨）**と**認証つきリクエストが必須**という2点を掴んでいなかった。
→ 「遅い・クォータを食う」ではなく**「素の `<img>` では表示できない」**が正しい。判断の強さが変わる。

### 判断として残すこと
- **単価の桁が3倍以上違う選択で、金額を客に提示する前**なら NotebookLM の deep research を回す価値がある（今回は結論が変わった）
- 逆に**実データから出た結論は変わらなかった**（Prismaのリレーション本数から出した「案bしかない」、現場がPSDを開くから出した「原本はDrive」）。**一次情報で決まっている部分は再調査しても動かない**

### 2026-09-03 追記：同じ内容を「内部正本」と「対外版」で持つときの調整点

HASSIN の制作システム設計は、**git=内部正本／Notion=先方が読む対外版**の2枚持ちにした。内容（構成・数字・結論）は同じで、文面だけ変える。調整したのは6箇所で、他案件でもそのまま使える。

| 内部の書き方 | 対外の書き方 | なぜ |
|---|---|---|
| 「阿部さんの問いへの回答」 | 「次回定例のたたき台としてご覧ください」 | 社内の依頼関係は先方に関係ない |
| **「先方へ提示する前に再確認」** | 「導入を決める段階でご確認ください」 | **「先方」は読み手自身を指す。対外文書に残ると意味が崩れる。最も事故りやすい** |
| 「論外」「主犯」「素人の作りものではなく」 | 「この用途には適しません」「容量が伸びやすい箇所」／削除 | クライアント側の担当者が作った物への評価になる |
| 「実装上の指摘」 | 「ご確認いただきたい点」 | 指摘ではなく共同で直す位置に置く |
| 「Neon招待が404、Vercel招待が阿部さん宛」 | 「①招待の再送 ②送付先のご確認をお願いします」＋付与済みの分は明記 | 詰まりの列挙ではなく依頼の形にする。できている分にも触れる |
| 「加藤（梨紗）さんからの追加論点」＋生の引用 | 「〜についてご相談をいただきました」＋要約 | バレンサー内の検討経緯と、担当者の作業メモは出さない |
| 「✕ 廃止」「採らなかった選択肢」 | 「採用しません」「検討したうえで採用しなかった案」 | 結論の強さは保ったまま、断じる語感を落とす |

**運用ルール**: 顧客フォルダの README に「内部正本とNotionは文面が違う／内容は同じ／片方直したら両方直す」を明記しておく。でないと次のセッションが Notion を正本と誤認して社内表現を書き足す。
