<?php
// ───────────────────────────────────────────────
// AI実装研修（リスキリング助成金）お問い合わせ受付
//  ・LPフォームから JSON で受ける
//  ・社内へメール通知（info@balencer.jp・全項目）
//  ・Notion DB（補助金/ヒトツリードと同一DB）へ登録。「流入元」で区別
...
$email   = trim($data['メール']       ?? '');
$tel     = trim($data['電話番号']     ?? '');
$people  = trim($data['受講予定人数'] ?? '');
$topic   = trim($data['相談したいこと'] ?? '');
$issue   = trim($data['現在の課題']   ?? '');
$other   = trim($data['その他ご要望'] ?? '');
$source  = trim($data['source']       ?? 'AI実装研修（リスキリング助成金）');
...
$body  = "AI実装研修（リスキリング助成金）LP よりお問い合わせがありました。\n";
$body .= str_repeat('-', 40) . "\n\n";
...
// ── ② Notion DB へ登録（流入元つき。補助金/ヒトツリードと同一DB） ──
